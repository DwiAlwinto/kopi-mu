document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('products', () => ({ //ini untuk data produts
        items : [
    ...
    Alpine.store('cart', { //keranjang belanja
        items: [],
        total: 0,
        quantity: 0,
        add(newItem){
            ...
            } else{ //kita cek barang sudah atau belum, cek apakah barang beda atau sama dengan di cart ya
                this.items = this.items.map((item) => {
                    //jika barang ya berbeda (ini akan nambah barang baru di bawahnya)
                    if (item.id !== newItem.id) {
                        return item;
                    }else{ //jika barang sudah ada maka quantity dan sub totalnya
                        item.quantity++;
        ...
        remove(id){
            ...
            } else if ( cartItem.quantity === 1) { //jika cartItem quantity sama dengan 1
